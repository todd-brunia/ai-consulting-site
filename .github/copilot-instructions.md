# Repository Instructions

Before proposing or implementing changes, read `AGENTS.md` and the applicable
documents in `docs/`. Use TypeScript, prefer React Server Components, reuse
existing components, and keep changes small and readable.

Do not invent clients, results, credentials, or consulting experience. Keep the
site practical, grounded, and low-hype. Explain business value before technical
details.

Code changes require an approved issue plan and the `approved-for-build` label.
Run lint, type checking, tests, and the production build before opening a pull
request. Never push directly to or merge into `main`.

Manual work may begin after `approved-for-build` on a non-`codex/issue-*`
branch. Applying `approved-for-ai-build` after that general approval is the
separate, explicit authorization for label-triggered Codex implementation.

For planning work, post the final proposal to the GitHub issue with
`scripts/post-issue-plan <issue-number>`. Do not treat a plan stored only in a
chat or prompt as approval. If GitHub access is unavailable, stop as blocked.
Keep initial plans concise and issue-specific. When changes are requested,
respond only to the new feedback unless the human explicitly asks for a
consolidated replacement plan.
